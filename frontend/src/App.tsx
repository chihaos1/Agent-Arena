...
function App() {
	...
	const [agentFinished, setAgentFinished] = useState<Record<string, boolean>>({
		strategy_a: false,
		strategy_b: false,
		strategy_c: false,
	})

	const allFinished = Object.values(agentFinished).every(Boolean)
				...
				decoder.decode(value)
					...
					.forEach(line => {
						...
						if (event.event !== "done") {
							...
							if (event.current_step === "completed" || event.current_step === "failed") {
								setAgentFinished(prev => ({ ...prev, [event.version_id]: true }))
								if (event.summary) {
									setAgentSummaries(prev => ({ ...prev, [event.version_id]: event.summary }))
								}
							}
